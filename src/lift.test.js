...
describe('lift', () => {
  ...
  beforeEach(() => {
    ...
    when(data).calledWith('settings', remarkConfig.settings).thenReturn({use});
    when(use).calledWith(legacyMarkerPlugin).thenReturn({use});
    when(use).calledWith(badgePlugin, badges).thenReturn({use});
  });
  ...
  it('should update the readme', async () => {
    when(use).calledWith(readmePlugin, documentation).thenReturn({process});
  ...
  it('should default `documentation` content to an empty object', async () => {
    when(use).calledWith(readmePlugin, {}).thenReturn({process});
    when(fs.readFile).calledWith(pathToReadmeFile, 'utf8').thenResolve(existingFileContents);
    when(process).calledWith(existingFileContents).thenReturn(updatedFileContents);
